#include "stdifm.h"
#include "porosity_diffusion.h"
#include <algorithm>    // need to make work std::max
...
IfmResult RegisterModule(IfmModule pMod)
{
  if (IfmGetFeflowVersion (pMod) < IFM_REQUIRED_VERSION)
    return False;
  g_pMod = pMod;
  IfmRegisterModule (pMod, "SIMULATION", "POROSITY_DIFFUSION", "porosity_diffusion", 0x1000);
  IfmSetDescriptionString (pMod, szDesc);
  IfmSetCopyrightPath (pMod, "porosity_diffusion.txt");
  IfmSetHtmlPage (pMod, "porosity_diffusion.htm");
  IfmSetPrimarySource (pMod, "porosity_diffusion.cpp");
...
{
  CPorosityDiffusion::FromHandle(pDoc)->Serialize (pDoc, pArc);
}
static void OnEditDocument (IfmDocument pDoc, Widget wParent)
{
  CPorosityDiffusion::FromHandle(pDoc)->OnEditDocument (pDoc, wParent);
}
static void PostSimulation (IfmDocument pDoc)
{
  CPorosityDiffusion::FromHandle(pDoc)->PostSimulation (pDoc);
}
static void PostTimeStep (IfmDocument pDoc)
{
  CPorosityDiffusion::FromHandle(pDoc)->PostTimeStep (pDoc);
}
...
static IfmResult OnBeginDocument (IfmDocument pDoc)
{
  ...
  try {
    IfmDocumentSetUserData(pDoc, new CPorosityDiffusion(pDoc));
  }
  catch (...) {
    return false;
  }
...
{
  delete CPorosityDiffusion::FromHandle(pDoc);
}

///////////////////////////////////////////////////////////////////////////
// Implementation of CPorosityDiffusion

// Constructor
CPorosityDiffusion::CPorosityDiffusion (IfmDocument pDoc)
  : m_pDoc(pDoc)
{
  /*
...
// Destructor
CPorosityDiffusion::~CPorosityDiffusion ()
{
  /*
   * TODO: Add your own code here ...
   */
...
CPorosityDiffusion* CPorosityDiffusion::FromHandle (IfmDocument pDoc)
{
  return reinterpret_cast<CPorosityDiffusion*>(IfmDocumentGetUserData(pDoc));
}
...
int	   g19_timestep_counter;					// counter for rock properties change timestep used for feflow screen message

void CPorosityDiffusion::Serialize (IfmDocument pDoc, IfmArchive pArc)
{
...
void CPorosityDiffusion::OnEditDocument (IfmDocument pDoc, Widget wParent)
{
...
void CPorosityDiffusion::PostSimulation (IfmDocument pDoc)
{
  /* 
   * TODO: Add your own code here ...
   */
...
void CPorosityDiffusion::PostTimeStep (IfmDocument pDoc)
{
	g3_deltat_double = atof(g3_deltat_string);
	g11_eff_timestep_double = atof(g11_eff_timestep_string);
	//	g13_PC_dt_double = atof(g13_PC_dt_string);    ANALYTICAL_1 FEFLOW; chemistry thus NO PHREEQC
...
calculate_new_por:										// goto label coming from line 209 (goto calculate_new_por;)
		...
		for (int elem = 0; elem < ne; elem++) {							// START of loop to visit all mesh elements for current species sp 
			...
			for (int nodeindex = 0; nodeindex < nDOF; nodeindex++) {	// START of loop to get the dissolved calcite value 'reaction product' from all nodes in an element 
				...
				if (g18_ddb == 0)											// 0 = dissolution, 1 precipitation, 2 both	
					...
					c[nodeindex] = IfmGetResultsTransportMassValue(pDoc, nn);   // ANALYTICAL 2
				else{
				//	if (g18_ddb == 1) c[nodeindex] = std::max(0.0, IfmGetRefDistrValue(pDoc, conc_ID, nn)); // MINERAL PRECIPITATION ANALYTICAL_3
				//	else { c[nodeindex] = IfmGetRefDistrValue(pDoc, conc_ID, nn); }   // BOTH: dissolution + precipitation ANALYTICAL_4
				}
			...
			//double rate_mine = aved_mine / g13_PC_dt_double;				// mineral dissolution rate in mol/(L*days) to extrapolate along effective phi/K timestep (g13) ANALYTICAL_5
			...
			double rate_mine;
			rate_mine = 1.7e-4*(6.0258 - aved_mine) * 86400 / 1000;	// ANALYTICAL_6 rate in 1/days to use in mol/L calculations 6.0258
			...
			double p = e_por + (e_init_por*e_Vm*rate_mine*g11_eff_timestep_double);// 'p' is the new porosity, g_7 is 1 for input in mol/L, calculate manually for inputs in mg/L PETRUS_5
			if (p > 0.99)
			{
				p = 0.99;
			}
	...
	/* 4 - Estimate and write new hydraulic conductivity (K), storage (S) and molecular diffusion (Dd) values based on previously changed porosity (p) values */
	...
	for (int elem = 0; elem < ne; elem++) {											// Careful! elem = 1 in FeFlow GUI is elem = 0 here 
		...
		IfmSetMatAnisotropyFactor2D(pDoc, elem, new_Anis);							// set anisotropy to 2D element (Kyy)

		double new_storage_coefficient = 1000 * 9.81 * (7.2e-9 + e_por * 1e-8);		// new storage
		IfmSetMatFlowCompressibility(pDoc, elem, new_storage_coefficient);			// set new storage coefficient

		double aa = pow(12.15*new_storage_coefficient/(2*new_Kxx),0.5);				// equals to (a) = (frequency * storage / 2 * transmissivity ) ^ 0.5
		double node_number = IfmGetNode(pDoc, elem, 1);
		double elem_X = IfmGetX(pDoc, node_number);
		double exponential = exp(-1 * elem_X *aa);
		double tide_amplitude = 1;												// MODIFY TIDAL AMPLITUDE		
		//double half_period = .517 / 2;											    // in days units
		//double new_velocity = (2.8284*new_Kxx*tide_amplitude*(1-exponential))/(elem_X*12.15*e_por*half_period);	// equation 10 article
		double new_velocity =( (2 * tide_amplitude*pow(12.15*new_storage_coefficient*new_Kxx, 0.5)) / (e_por*3.1416))*exponential;	// equation 10 article
		double new_equivalent_diffusion = 8.64e-5 + 0.1* new_velocity;				// multiplies by dispersivity 0.1 m 
		IfmSetMatMassDiffusion(pDoc, elem, new_equivalent_diffusion);				// sets new equivalent diffusion
	...
	if (g6_message > 1);			// IF g6_messages > 1, do nothing.
			...
			g19_timestep_counter = g19_timestep_counter + 1;
			IfmInfo(pDoc, "Update: %i Eff.sim time: %g days, p,K,S,Dd update took %g clock() clicks or %g secs", g19_timestep_counter, g12_accum_phiK, clock_time, timeInSeconds);
		} // FeFlow log screen message 
